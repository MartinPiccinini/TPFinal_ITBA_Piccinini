    ...
    while True:
        ...
        Cerrar: Cierra la aplicación.
            ...
            case "pedir datos": # entra en el bloque de pedir datos a la api de polygon
                ...
                dicc, q_count  = f_main.get_stock_prices(tick,desde,hasta,key_api) # se guarda en un diccionario el json resultante del request a la api
                
                if q_count > 0: #chequea que el request haya traido datos
                    ...
                    print(f'No existen datos para el ticker {tick}, pruebe con otro ticker.\n')
            ...
            case "tabla": # comando para imprimir una tabla con los datos del ticker solicitado
                if hay_tickers: # aqui el booleano creado en la linea 9 o actualizado en la linea 44 hace que se entre en el codigo que imprimime la tabla o muestre un mensaje de error
                    ...
                    if tick_v in l_tickers:                                  # se chequea si el ticker existe en la base
                        ...
                        print(f_sql.query_ticker_close(tick_v)) # si existe, se imprime la tabla con los datos presentes en la base de datos
            ...
            case "graficar":
                if hay_tickers: # aqui el booleano creado en la linea 9 o actualizado en la linea 44 hace que se entre en el codigo que grafica o muestre un mensaje de error
                    input_graph = input("¿Qué tipo de gráfico quiere ver? Evolutivo o Velas: ").lower()
                    match input_graph:
                        case 'evolutivo':
                            tick_v = input('Ingrese el ticker a graficar: ').upper() # se pide el ticker, tambien llevando a mayuscula
                            if tick_v in l_tickers:   # se chequea si el ticker existe en la base
                                
                                df_viz = f_sql.query_ticker_close(tick_v) # si existe, se grafican los datos presentes en la base de datos
                        ...
                        case 'velas':
                            ...
                            if tick_v in l_tickers:   # se chequea si el ticker existe en la base
                                ...
                                df_viz = f_sql.query_ticker_all(tick_v) # si existe, se grafican los datos presentes en la base de datos
                                
                                f_main.candlestick_plot(df_viz,tick_v)  # si existe, se grafican los datos presentes en la base de datos
                            else:                                   # si no existe, se imprime un mensaje que se debe pedir datos del ticker solicitado 
                                print('\nEl ticker solicitado no se encuentra en la base de datos, solicite los datos con el comando "Pedir datos"\n')
                       
                        case default:
                                print('No existe ese tipo de gráfico, prueba con "Evolutivo" o "Velas".')
